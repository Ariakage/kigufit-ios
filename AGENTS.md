...
- **P2**：剖面可视化、头壳档案管理/导入 UI、头壳放大试算（按海绵配置输出增量/缩放建议）、打磨
- **P3（进行中）**：本地 OBJ 分析引擎 ✅（App 内解析/自动判向/内腔剖面/眼孔/壁厚 → kigufit.shell/v1 档案）；BYOK LLM 流水线 ✅（Keychain 存 Key、记录一键 AI 解读、仅发送测量数值与判定，支持 DeepSeek/OpenAI 兼容自定义）
